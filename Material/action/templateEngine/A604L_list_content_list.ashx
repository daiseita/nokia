...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        oValue.setRequestGet("SearchTxt,PageNum,ThisPage,A12I02UV0010");
        ...
        if (oValue.Data("A12I02UV0010") != "")
        {
            TableString = "select COUNT(A68I04JJA31I02)cnt ,A31F01NV0064  from A68 left join A31 on A68I04JJA31I02 = A31I02UV0010  where (A68I11CV0001='C' or A68I11CV0001='D' or A68I11CV0001='E') and A68I13JJA12I02='" + oValue.Data("A12I02UV0010") + "' and A68I09CV0001='' group by A31F01NV0064";
        }
        else {
            TableString = "select COUNT(A68I04JJA31I02)cnt ,A31F01NV0064  from A68 left join A31 on A68I04JJA31I02 = A31I02UV0010 where (A68I11CV0001='C' or A68I11CV0001='D' or A68I11CV0001='E') and A68I09CV0001=''  group by A31F01NV0064";
        }        
        Sql.SetColumnName("cnt,A31F01NV0064");
        Sql.SetTableName("( " + TableString + " )A68 ");
        //Sql.sqlTransferColumn = "A30I03CV0001";
        Sql.SetOrderBy("A31F01NV0064", false);
        Sql.RecordCountQuery("A31F01NV0064");
        dt = Sql.Query("A68", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A68"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A68"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A68"].Columns[s].ColumnName;

                    if (ColumnName == "A68INA")
                    {
                        string A68INA = DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A68"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A68INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A68"].Rows[i][s].ToString() });
                    }
        ...
        /* 事件等級select */
        string SQL_s1 = "  select A12I02UV0010,A12F01NV0064 from A12 where A12I03CV0001='B'";
        Sql.ClearQuery();
        dt = Sql.selectTable(SQL_s1, "A12");
        if (dt.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < DataSet2.Tables["A12"].Rows.Count; i++)
            {
                select01_Tittile.Add(new TittleSelect { value = DataSet2.Tables["A12"].Rows[i]["A12F01NV0064"].ToString() });
                select01_Value.Add(new ValueSelect { value = DataSet2.Tables["A12"].Rows[i]["A12I02UV0010"].ToString() });
            }
