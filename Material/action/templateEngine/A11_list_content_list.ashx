...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A11F01NV0032", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A11F02NV0256", "like", oValue.Data("SearchTxt"), true);            
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A11I01XA,A11I02UV0006,A11I03CV0001,A11F01NV0032,A11F02NV0256,A11IND,A11INT,A11INA,A11INU,A11UPD,A11UPT,A11UPA,A11UPU,A11UPC from A11";
        Sql.SetColumnName("A11I01XA,A11I02UV0006,A11I03CV0001,A11F01NV0032,A11F02NV0256,A11IND,A11INT,A11INA,A11INU,A11UPD,A11UPT,A11UPA,A11UPU,A11UPC");
        Sql.SetTableName("( " + TableString + " )A11 ");
        Sql.sqlTransferColumn = "A11I03CV0001";
        Sql.SetOrderBy("A11I02UV0006", false);
        Sql.RecordCountQuery("A11I02UV0006");
        dt = Sql.Query("A11", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A11"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A11"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A11"].Columns[s].ColumnName;

                    if (ColumnName == "A11INA")
                    {
                        string A11INA = DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A11"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A11INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A11"].Rows[i][s].ToString() });
                    }
