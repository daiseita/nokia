...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A30F01NV0064", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A30F02NV0512", "like", oValue.Data("SearchTxt"), true);            
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A30I01XA,A30I02UV0004,A30I03CV0001,A30F01NV0064,A30F02NV0512,A30IND,A30INT,A30INA,A30INU,A30UPD,A30UPT,A30UPA,A30UPU,A30UPC from A30";
        Sql.SetColumnName("A30I01XA,A30I02UV0004,A30I03CV0001,A30F01NV0064,A30F02NV0512,A30IND,A30INT,A30INA,A30INU,A30UPD,A30UPT,A30UPA,A30UPU,A30UPC");
        Sql.SetTableName("( " + TableString + " )A30 ");
        Sql.sqlTransferColumn = "A30I03CV0001";
        Sql.SetOrderBy("A30I02UV0004", false);
        Sql.RecordCountQuery("A30I02UV0004");
        dt = Sql.Query("A30", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A30"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A30"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A30"].Columns[s].ColumnName;

                    if (ColumnName == "A30INA")
                    {
                        string A30INA = DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A30"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A30INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A30"].Rows[i][s].ToString() });
                    }
