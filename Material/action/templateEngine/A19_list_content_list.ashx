...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A19F01NV0032", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A19F02NV0256", "like", oValue.Data("SearchTxt"), true);            
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A19I01XA,A19I02UV0004,A19I03CV0001,A19F01NV0032,A19F02NV0256,A19F03NV0256,A19IND,A19INT,A19INA,A19INU,A19UPD,A19UPT,A19UPA,A19UPU,A19UPC from A19";
        Sql.SetColumnName("A19I01XA,A19I02UV0004,A19I03CV0001,A19F01NV0032,A19F02NV0256,A19F03NV0256,A19IND,A19INT,A19INA,A19INU,A19UPD,A19UPT,A19UPA,A19UPU,A19UPC");
        Sql.SetTableName("( " + TableString + " )A19 ");
        Sql.sqlTransferColumn = "A19I03CV0001";
        Sql.SetOrderBy("A19I02UV0004", false);
        Sql.RecordCountQuery("A19I02UV0004");
        dt = Sql.Query("A19", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A19"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A19"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A19"].Columns[s].ColumnName;

                    if (ColumnName == "A19INA")
                    {
                        string A19INA = DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A19"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A19INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A19"].Rows[i][s].ToString() });
                    }
