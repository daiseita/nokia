...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A60INA", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A60F02NV0128", "like", oValue.Data("SearchTxt"), true);                
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A60I01XA,A60I02UV0012,A60I03CV0001,A60I05CV0032,A60F01NV0064,A60F02NV0128,A60IND,A60INT,A60INA from A60 where A60I03CV0001='L'";
        Sql.SetColumnName("A60I01XA,A60I02UV0012,A60I03CV0001,A60I05CV0032,A60F01NV0064,A60F02NV0128,A60IND,A60INT,A60INA");
        Sql.SetTableName("( " + TableString + " )A60 ");
        //Sql.sqlTransferColumn = "A60I03CV0001";
        Sql.SetOrderBy("A60INA", true);
        Sql.RecordCountQuery("A60I02UV0012");
        dt = Sql.Query("A60", ThisPage, PageNum);
        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A60"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A60"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A60"].Columns[s].ColumnName;

                    if (ColumnName == "A60INA")
                    {
                        string A60INA = DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A60"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A60INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A60"].Rows[i][s].ToString() });
                    }
