...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A65INA", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A65F02NV0128", "like", oValue.Data("SearchTxt"), true);                
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A65I01XA,A65I02UV0012,A65I03CV0001,A65F01NV0064,A65F02NV0128,A65IND,A65INT,A65INA from A65 where A65I03CV0001='G'";
        Sql.SetColumnName("A65I01XA,A65I02UV0012,A65I03CV0001,A65F01NV0064,A65F02NV0128,A65IND,A65INT,A65INA");
        Sql.SetTableName("( " + TableString + " )A65 ");
        //Sql.sqlTransferColumn = "A65I03CV0001";
        Sql.SetOrderBy("A65INA", true);
        Sql.RecordCountQuery("A65I02UV0012");
        dt = Sql.Query("A65", ThisPage, PageNum);
        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A65"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A65"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A65"].Columns[s].ColumnName;

                    if (ColumnName == "A65INA")
                    {
                        string A65INA = DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A65"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A65INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A65"].Rows[i][s].ToString() });
                    }
