...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A71INA", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A71F02NV0128", "like", oValue.Data("SearchTxt"), true);                
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A71I01XA,A71I02UV0010,A71I03CV0001,A71F01NV0064,A71F02NV0128,A71F03NT,A71IND,A71INT,A71INA from A71";
        Sql.SetColumnName("A71I01XA,A71I02UV0010,A71I03CV0001,A71F01NV0064,A71F02NV0128,A71F03NT,A71IND,A71INT,A71INA");
        Sql.SetTableName("( " + TableString + " )A71 ");
        //Sql.sqlTransferColumn = "A71I03CV0001";
        Sql.SetOrderBy("A71INA", true);
        Sql.RecordCountQuery("A71I02UV0010");
        dt = Sql.Query("A71", ThisPage, PageNum);
        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A71"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A71"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A71"].Columns[s].ColumnName;

                    if (ColumnName == "A71INA")
                    {
                        string A71INA = DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(6, 2) + " " + DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A71"].Rows[i]["A71INA"].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A71INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A71"].Rows[i][s].ToString() });
                    }
