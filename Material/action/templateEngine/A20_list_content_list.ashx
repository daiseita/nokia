...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (oValue.Data("SearchTxt") != "")
        {
            ...
            Sql.SetWhere("or", "A20F01NV0006", "like", oValue.Data("SearchTxt"), true);            
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        Sql.RecordCountQuery("A20I02UV0010");
        dt = Sql.Query("A20", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A20"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A20"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A20"].Columns[s].ColumnName;

                    if (ColumnName == "A20INA")
                    {
                        string A20INA = DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A20"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A20INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A20"].Rows[i][s].ToString() });
                    }
