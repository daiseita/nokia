...
public class A10_list_content_list : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Sql.SetWhere("or", "A10I02UV0064", "like", oValue.Data("SearchTxt"), true);
            Sql.SetWhere("or", "A10F02NV0032", "like", oValue.Data("SearchTxt"), true);            
        }
        string TableString = "";
        //Sql.ClearQuery();
        ...
        TableString = "select A10I01XA,A10I02UV0064,A10I03JJA20I02,A10I04CV0001,A10F01CV0032,A10F02NV0032,A10F03CV0015,A10A01,A10IND,A10INT,A10INA,A10INU,A10UPD,A10UPT,A10UPA,A10UPU,A10UPC from A10";
        Sql.SetColumnName("A10I01XA,A10I02UV0064,A10I03JJA20I02,A10I04CV0001,A10F01CV0032,A10F02NV0032,A10F03CV0015,A10A01,A10IND,A10INT,A10INA,A10INU,A10UPD,A10UPT,A10UPA,A10UPU,A10UPC");
        Sql.SetTableName("( " + TableString + " )A10 ");
        Sql.sqlTransferColumn = "A10I04CV0001";
        Sql.SetOrderBy("A10I02UV0064", false);
        Sql.RecordCountQuery("A10I02UV0064");
        dt = Sql.Query("A10", ThisPage, PageNum);

        if (dt != null)
        {
            DataSet DataSet1 = new DataSet();
            DataSet1.Tables.Add(dt);
            for (int i = 0; i < DataSet1.Tables["A10"].Rows.Count; i++)
            {
                List<ValueInput> Input01_Value = new List<ValueInput>();
                for (int s = 0; s < DataSet1.Tables["A10"].Columns.Count; s++)
                {
                    string ColumnName = DataSet1.Tables["A10"].Columns[s].ColumnName;

                    if (ColumnName == "A10INA")
                    {
                        string A10INA = DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(0, 4) + "/" + DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(4, 2) + "/" + DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(6, 2) + " " + DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(8, 2) + ":" + DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(10, 2) + ":" + DataSet1.Tables["A10"].Rows[i][s].ToString().Substring(12, 2);
                        Input01_Value.Add(new ValueInput { value = A10INA });
                    }
                    else
                    {
                        Input01_Value.Add(new ValueInput { value = DataSet1.Tables["A10"].Rows[i][s].ToString() });
                    }
