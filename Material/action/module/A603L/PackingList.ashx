...
public class Info : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        SQL = "select DISTINCT A68IND from A68 where A68I07CV0001='"+ Sort  +"' order by A68IND desc";
        dt = Sql.selectTable(SQL, "A68");
        oTemplate.SetTemplatesDir(pb.ActionTemplateName + "module/");
        oTemplate.SetTemplateFileCharset("A603L_PackingList_content.html", "UTF-8");
        ...
        if (dt != null)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                oTemplate.SetVariable_HTML_OPTION("A68IND", A68IND, "", dt.Rows[i][0].ToString());
            }
        }
        oTemplate.SetVariable_HTML_SELECT("A68IND");
