...
public class A60_face : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        oValue.setRequestPost("Action,OrderDataString,A67I01XA,A67F02NV0128,A61I13JJA12I02,A60F02NV0128");
        ...
        oTemplate.SetVariable_HTML_SELECT("A61I04JJA31I02");
        oTemplate.SetVariable_HTML_SELECT("A61I20CV0020");
    ...
    public void OrderDataOperate(HttpContext context)
    {
        ...
        if (pContent != "")
        {
            ...
            for (int i = 0; i < OrderItem.Length; i++)
            {
                ...
                string strA119I02 = ItemData[0].Substring(20, 4);
                string strPack = ItemData[0].Substring(24, (ItemData[0].Length -24));
                string strNum = ItemData[1];

                if (codeArray.ContainsKey(strPack))
                {                    
                }
                else
                {
                    codeArray.Add(strPack, strA12I02C);
                }
                string pSum = codeArray[strPack].ToString();

                //strSQL += OrderItemAdd(ItemData[0], ItemData[1], A65I02UV0012);
            }
            string PackWhere = "";
            /* 將供應商&承包商暫存於datatable */
            foreach (KeyValuePair<string, string> item in codeArray)
            {
                if (PackWhere != "") { PackWhere = PackWhere + " or "; }
                PackWhere = " A66I20CV0020 ='" + item.Key.ToString() + "' "; 
            }
            Sql.ClearQuery();
            SQL = "select top 1 A66I02UV0019,A66I20CV0020 from A66 where A66 ( " + PackWhere + " ) and A66I11CV0001<>'' ";
            dt = Sql.selectTable(SQL, " A66o");
            if (dt != null)
            {
                context.Response.Write("alert('" + Ms.Msg28 + "--" + dt.Rows[0]["A66I20CV0020"].ToString() + "');");
                return;
            }
            dt = null;
            foreach (KeyValuePair<string, string> item in codeArray)
            {
                Sql.ClearQuery();
                SQL = "select  A66I02UV0019,A66I04JJA31I02,A66I05JJA19I02,A66I12JJA12I02,A66I09CV0001,A66I10JJA66I02,A66F01NV0064 from A66 where A66I20CV0020='" + item.Key.ToString() + "' ";
                dt = Sql.selectTable(SQL, "A66" + item.Key.ToString());
                if (dt != null)
                {
                    for (int r = 0; r < dt.Rows.Count; r++)
                    {
                        ...
                        strSQL += setTimeA62(dt.Rows[r]["A66I02UV0019"].ToString());
                        /* A61b資料填入 */
                        strSQL += setDataA61(dt.Rows[r]["A66I02UV0019"].ToString(), dt.Rows[r]["A66I12JJA12I02"].ToString(), item.Value.ToString());
                        strSQL += OrderItemAdd(A67I02UV0012, dt.Rows[r]["A66I02UV0019"].ToString(), dt.Rows[r]["A66I12JJA12I02"].ToString(), item.Value.ToString(), item.Key.ToString(), dt.Rows[r]["A66I05JJA19I02"].ToString(), dt.Rows[r]["A66I09CV0001"].ToString(), dt.Rows[r]["A66I10JJA66I02"].ToString(), dt.Rows[r]["A66F01NV0064"].ToString());
                        strSQL += setDataA66(dt.Rows[r]["A66I02UV0019"].ToString(), item.Value.ToString());
                    }
                }
                dt = null;
            }
    ...
    public string OrderItemAdd(string A67I02UV0012, string A68I02UV0019, string A61I12JJA12I02, string A61I13JJA12I02, string A66I20CV0020, string A66I05JJA19I02, string A66I09CV0001, string A66I10JJA66I02, string A66F01NV0064) 
    {
        DataTable dt = new DataTable();
        bool IsChild = false;
        string strSQL = "";
